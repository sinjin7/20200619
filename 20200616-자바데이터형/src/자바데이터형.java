 ...
 *		컴퓨터는 0,1 => bit
 *	정수
 *	= byte (0,1 을 8개를 저장하는 공간) => 8bit(1byte) => web 주로 사용
 *	  11111111 = 음수 ==> -128
 *	  01111111 = 양수 ==> 127  ====>-128~127 저장
 *		==> 파일읽기,파일쓰기, 파일업로드,파ㅣㄹ다울로드,네트워크 전송
 *	= short (0,1 을 =>16 개를 저장하는 공간) => 16bit(2byte)
 *		1111 1111 1111 1111 => -32768
 *		0111 1111 1111 111 => 32767
 *		==> C언어와 호환성 (사용빈도가 없다)
 ...
 *	***문제점
 *		float f=10.5 (10.5 실수를 f 에 넣어라. 10.5를 먼저 인식함.) 뒤에부터 대입하기
 *			저장공간 = 첨부할 값
 *			============== 
 ...
 *				
 *				정수  < 실수
 *정수보다 표현을 더 할수 있어서 실수가 더큼 . 실수가 정수보다 표현법이 더 많다. 소숫점자리 표현 
 *====================================================================
 *	변수 : 한개의데이터를 저장하는 메모리 공간의 별칭
 *	변수명 제작
 *		1)알파벳이나 한글로 시작한다
 *		 ===== 알파벳은 반드시 대소문자 구분
 * 			a != A
 * 		2) 숫자 사사용이 가능 (단 앞에는 사용할 수 없다)
 * 			a1,a2...(2aerror))
 * 		3) 특수문자 사용이 가능 ( _ , $ )
 * 		  _ => 임시파일, 단어가 여러개 일 경우에 주로 사용
 * 			 file_size , file_name...
 * 		4) 키워드( 자바에서 사용중인 단어)는 사용 불가능
 * 			int int1
 ...
 * 
 * 			점수를 저장 => int jumsu;
 * 						jumsu=100;	// 키보드로 입력을 받거나, 파일에 있는 데이터 일기
 * 
 * 						int jumsu=100;	// 직접 입력
 *			여러개
 *				int a=0;
 ...
 *				========
 *					int a,b,c;
 *					int a=0, b=0, c=0;  int 가 동일 할때 사용가능 
 *				==========================================
 *
 *		변수에 초기화
 *		1. 정수형
